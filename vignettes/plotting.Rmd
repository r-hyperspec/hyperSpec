...
```{block, type="note", echo=TRUE}
**NOTE.**
Functions `plot_map()`{.r}, `plotvoronoi()`{.r}, and `levelplot()`{.r} are package **lattice** functions.
...
Function `plotvoronoi()`{.r} is a special version of `plot_map()`{.r} that produces Voronoi diagram of the `hyperSpec` object (Fig. \@ref(fig:voronoi)).
...
Methods `plot_map`{.r}, `plotvoronoi`{.r}, `levelplot`{.r}, and `plot_c`{.r} use package **lattice** functions.
