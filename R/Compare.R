...
#'
#' The comparison operators `>`, `<`, `>=`, `<=`, `==`, and `!=` work on the
#' spectra matrix of the `hyperSpec` object. They have their usual meaning
#' (see [base::Comparison()]).
...
#' `hyperSpec` object.
#'
#' With numeric vectors [hyperSpec::sweep()] might be more appropriate.
#'
...
#' @docType methods
#'
#' @param e1,e2 Either two `hyperSpec` objects or one `hyperSpec` object and
#'        matrix of same size as `hyperSpec[[]]` or a scalar
#'        (numeric of length 1).
#'
...
#'
#' @seealso
#' - [hyperSpec::sweep-methods()] for calculations involving a vector and
#'   the spectral matrix.
