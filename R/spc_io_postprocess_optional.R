...
#' All file import filters should call `.spc_io_postprocess_optional()` to
...
hySpc.testthat::test(.spc_io_postprocess_optional) <- function() {
  ...
  test_that("removing of zero/NA spectra", {
    ...
    expect_equivalent(
      .spc_io_postprocess_optional(tmp,
        file.remove.emptyspc = TRUE,
        file.keep.name = FALSE
      ),
      tmp[-c(2, 3, 5)]
    )
  })
  ...
  test_that("filenames", {
    flu$filename <- NULL
    tmp <- .spc_io_postprocess_optional(
      flu,
      filename = "test",
      file.remove.emptyspc = FALSE,
      file.keep.name = TRUE
    )
